...
*** DONE 32. Use =__getattr__=, =__getattribute__=, and =__setattr__= for Lazy Attributes
    CLOSED: [2019-07-24 Wed 17:05]
    - State "DONE"       from "IN-PROGRESS" [2019-07-24 Wed 17:05]
    :LOGBOOK:
    CLOCK: [2019-07-24 Wed 16:28]--[2019-07-24 Wed 17:05] =>  0:37
    :END:
    
    Make a =LazyDB= class that mutates itself on =__getattr__=.
    ...
    Here, the =exists= attribute is present in the instance dictionary, so =__getattr__= is never called. The =foo= attribute is not, so =__getattr__= is invoked the first time, but not the second.
   
    But what if you did want to have something called each time the attribute is accessed?
    ...
    What if you want to block for attributes you know are bad?
    ...
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
      File "/var/folders/bz/tc86r39x3yb0dm00h93xbbn40000gn/T/babel-uQyA0l/python-QLh71y", line 18, in <module>
        print('bad_name: ', data.bad_name)
      File "/var/folders/bz/tc86r39x3yb0dm00h93xbbn40000gn/T/babel-uQyA0l/python-QLh71y", line 8, in __getattr__
        raise AttributeError('%s is missing' % name)
    AttributeError: bad_name is missing
    #+end_example

    How can we check for attributes before we use them?
    ...
    Notice again that =__getattr__= is called once. If we were to implement =__getattribute__=, it would be called each time =hasattr= is called.

    How does =__setattr__= work in constrast?
    ...
    =__setattr__= is called every time an attribute is set, unlike =__getattr__=. 

    The problem with =__setattr__= and =__getattribute__= is that they are called on every attribute access. You may not want that to happen.
   ...
   Called __getattribute__(_data)
    ...
    Traceback (most recent call last):
      File "<stdin>", line 1, in <module>
      File "/var/folders/bz/tc86r39x3yb0dm00h93xbbn40000gn/T/babel-uQyA0l/python-p9lrM9", line 11, in <module>
        data.foo
      File "/var/folders/bz/tc86r39x3yb0dm00h93xbbn40000gn/T/babel-uQyA0l/python-p9lrM9", line 7, in __getattribute__
        return self._data[name]
      File "/var/folders/bz/tc86r39x3yb0dm00h93xbbn40000gn/T/babel-uQyA0l/python-p9lrM9", line 7, in __getattribute__
        return self._data[name]
      File "/var/folders/bz/tc86r39x3yb0dm00h93xbbn40000gn/T/babel-uQyA0l/python-p9lrM9", line 7, in __getattribute__
    ...
    We can fix this recursion issue like this.
    ...
    *Takeaways*
    ...
    - Understand that =__getattr__= only gets called once when accessing a missing attribute, whereas =__getattribute__= gets called everytime an attribute is accessed.
...
*** DONE 33. Validate Subclasses with Metaclasses
    CLOSED: [2019-08-07 Wed 14:44]
    - State "DONE"       from "IN-PROGRESS" [2019-08-07 Wed 14:44]
    :LOGBOOK:
    CLOCK: [2019-08-07 Wed 14:31]--[2019-08-07 Wed 14:44] =>  0:13
    :END:
    ...
    #+RESULTS:
    : (<class '__main__.Meta'>, 'MyClass', (<class 'object'>,), {'__module__': '__main__', '__qualname__': 'MyClass', 'stuff': 123, 'foo': <function MyClass.foo at 0x1058d89d8>})

    Provide validation via a metaclass
    ...
    #+begin_src python :session :results output :exports both
      ...
      class Triangle(Polygon):   # this is OK
          sides = 3
    ...
    #+RESULTS:
    #+begin_example
    /var/folders/bz/tc86r39x3yb0dm00h93xbbn40000gn/T/babel-aIbZBs/python-nbHcHq in <module>
         18 
         19 print('Before class')
    ---> 20 class Line(Polygon):
         21     print('Before sides')
         22     sides = 1

    /var/folders/bz/tc86r39x3yb0dm00h93xbbn40000gn/T/babel-aIbZBs/python-nbHcHq in __new__(meta, name, bases, class_dict)
          4         if bases != (object,):
    ...
    *Takeaways*
    ...
    - Metaclasses have slightly different syntax in v2 & v3
    - The =__new__= method of metaclasses is run after the =class= statement's entire body has been processed

    
*** DONE 34. Register Class Existence with Metaclasses
    CLOSED: [2019-08-26 Mon 15:59]
    - State "DONE"       from "IN-PROGRESS" [2019-08-26 Mon 15:59]
    :LOGBOOK:
    CLOCK: [2019-08-26 Mon 15:25]--[2019-08-26 Mon 15:59] =>  0:34
    CLOCK: [2019-08-26 Mon 15:12]--[2019-08-26 Mon 15:23] =>  0:11
    :END:

    A serialize/deserialize example
    ...
    This works for 2D point types, but does not really extend to other classes. Since other classes would likely need serialize/deserialize capabilities, this becomes inflexible.

    We can maintain a mapping to the classes with a registry.
    ...
    This works on more classes, but if you forget to =register_class=, code will break at runtime by referencing a non-existant key.
    
    How can we ensure all the classes that we want to have this functionality gets it?
    ...
    *Takeaways*
    ...
    - Class registration is a helpful patter for building modular Python programs
...
** Time
 ...
 #+CAPTION: Clock summary at [2019-08-26 Mon 16:00]
 | Headline                                       | Time    |      |      |
 |------------------------------------------------+---------+------+------|
 | *Total time*                                   | *12:41* |      |      |
 |------------------------------------------------+---------+------+------|
 ...
 | \_  Metaclasses and Attributes                 |         | 3:11 |      |
 | \_    29. Use Plain Attributes Instead of...   |         |      | 0:39 |
 | \_    30. Consider =@property= Instead of...   |         |      | 0:21 |
 | \_    31. Use Descriptors for Reusable...      |         |      | 0:36 |
 | \_    32. Use =__getattr__=,...                |         |      | 0:37 |
 | \_    33. Validate Subclasses with Metaclasses |         |      | 0:13 |
 | \_    34. Register Class Existence with...     |         |      | 0:45 |
 #+END:
